...
CREATE FUNCTION postgis_scripts_build_date() RETURNS text
    ...
    AS $$SELECT '2012-10-07 12:25:35'::text AS version$$;
...
CREATE TABLE sensitive_occurrences (
    ...
    CONSTRAINT enforce_srid_sensitive_location CHECK ((st_srid(sensitive_location) = 4326))
);
...
CREATE TABLE vettings (
    ...
    area geometry,
    CONSTRAINT enforce_dims_area CHECK ((st_ndims(area) = 2)),
    CONSTRAINT enforce_geotype_area CHECK (((geometrytype(area) = 'MULTIPOLYGON'::text) OR (area IS NULL))),
    CONSTRAINT enforce_srid_area CHECK ((st_srid(area) = 4326))
);
